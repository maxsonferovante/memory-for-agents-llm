...
- `PostToolUse` -> `python3 hooks/memory_event_poster.py --source <agent-hook-source>`
- `SubagentStop` -> `python3 hooks/memory_event_poster.py --source codex-subagent-hook --event-type subagent_stop`
- `Stop` -> `python3 hooks/memory_event_poster.py --source <agent-hook-source> --event-type session_stop`
- `Stop` -> `python3 hooks/memory_hooks.py promote-ready --queue knowledge/_proposals`

Claude Code uses an installed runner generated by `scripts/install_claude_assets.py`. Codex uses the repo-local `.codex/hooks.json` and `hooks/codex_hook_runner.py`.
