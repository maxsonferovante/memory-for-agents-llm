...
Claude Code uses an installed runner generated by `scripts/install_claude_assets.py`. Codex uses either the repo-local `.codex/hooks.json` or the global `~/.codex/hooks.json` installed by `scripts/install_codex_assets.py`. Both call `hooks/codex_hook_runner.py` or its installed copy.
