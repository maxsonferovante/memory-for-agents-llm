...
- `scripts/install_copilot_assets.py` installs GitHub Copilot instructions, prompts, agents, skills, the Spec Memory event workflow, and the workflow helper script into a target repository. It supports `--target-repo`, `--dry-run`, `--force`, and optional `--stack-host` for the workflow ingest fallback.
